...
<nav class="flex fixed p-1 md:p-2 z-30 top-0 w-full bg-white flex-wrap dark:bg-gray-700 border-b border-gray-200 dark:border-gray-500">

<a class="flex font-medium md:font-semibold text-lg items-center dark:text-white justify-center text-gray-700" href="/"><img src="https://kudadam.sirv.com/logo/logo.png?q=5" class="h-10" alt=""><span class="hidden md:inline">Kudadam</span></a>
		...
		<ul class="md:flex">
			{#each nav_links as { name , url, id }  (id) }
				<li class="p-2 text-base text-gray-700 text-lg font-semibold">
					<a href={url} sveltekit:prefetch class="text-gray-700 block" class:active={segment === `/${name.toLowerCase()}`}>{name}</a>
				</li>
			{/each}
		<slot>
		</slot>
